...
# --- 1. CONFIGURATION ---
BATCH_SIZE = 128        # Tiny models use less memory, so we can increase batch size
...
print(f"Distilling on {DEVICE} with T={TEMPERATURE} and Alpha={ALPHA}")

# --- 2. DATA LOADERS (CIFAR-100) ---
# Must match the Teacher's expected input size (224x224)
...
optimizer = optim.AdamW(student.parameters(), lr=LR) # AdamW is usually better for Transformers
...
torch.save(student.state_dict(), "vit_tiny_cifar100_distilled.pth")
