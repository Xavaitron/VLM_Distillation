...
# --- 1. CONFIGURATION ---
BATCH_SIZE = 64          # Reduced since running both teacher and student
...
print(f"Distilling ResNet-152 -> ResNet-18 on {DEVICE}")
...
# B. Load STUDENT (ResNet-18, Trainable)
print("Loading Student (ResNet-18)...")
...
print(f"Student (ResNet-18) Parameters: {s_params:,}")
print(f"Compression Ratio: {t_params/s_params:.1f}x")
print("Same-Architecture Distillation: CNN -> CNN")
...
for epoch in range(EPOCHS):
    ...
    if acc > best_acc:
        best_acc = acc
        torch.save(student.state_dict(), "resnet18_cifar100_distilled.pth")

print(f"\nBest Distilled ResNet-18 Accuracy: {best_acc:.2f}%")
print("Distilled Student Saved as 'resnet18_cifar100_distilled.pth'!")
