...
for epoch in range(1,epochs+1):
    for step,(X,y) in enumerate(trainloader):
        ...
        adv_logits, inputs_adv = rslad_inner_loss_xadv(student,teacher_logits,X,y,optimizer,step_size=2/255.0,epsilon=epsilon,perturb_steps=10)
        with torch.no_grad():
            delta = inputs_adv - X
        ...
        diff_grad_kl_loss = criterion_kl(F.log_softmax(student_plus - student_minus, dim=1), F.softmax((teacher_plus - teacher_minus).detach(), dim=1))

        kl_Loss1 = kl_loss(torch.log(F.softmax(adv_logits,dim=1)),F.softmax(teacher_logits.detach(),dim=1))
        kl_Loss2 = kl_loss(torch.log(F.softmax(student_logits,dim=1)),F.softmax(teacher_logits.detach(),dim=1))
    ...
    #        param_group['lr'] *= 0.1
    if epoch in [int(epochs*0.7),int(epochs*0.86),int(epochs*0.95)]:
        for param_group in optimizer.param_groups:
            param_group['lr'] *= 0.1
