...
def hat_loss(model, x, y, optimizer, step_size=0.007, epsilon=0.031, perturb_steps=10, h=3.5, beta=1.0, gamma=1.0, 
    ...
    out_clean, out_adv, out_help, out_minus = model(x), model(x_adv), model(x_hr), model(x  - (x_adv - x))
    ...
    # loss_adv = (1/len(x)) * criterion_kl(F.log_softmax(out_adv + out_minus, dim = 1),F.softmax(2*out_clean,dim =1 ))
    # loss_adv = nn.MSELoss()(out_adv + out_minus, 2*out_clean)
