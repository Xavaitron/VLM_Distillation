...
if args.teacher == "LTD":
    ...
    state_dict = torch.load('./result_models/adaad_0_0_0_100_BDM2024-03-25RES-18.pt')
    teacher.load_state_dict(state_dict)
elif args.teacher == "SELF2":
    teacher = resnet18()
    teacher = torch.nn.DataParallel(teacher)
    state_dict = torch.load('./result_models/adaad_0_0_0_100_SELF2024-03-27RES-18.pt')
    teacher.load_state_dict(state_dict)
else:
    pass
...
for epoch in range(1,epochs+1):
    for step,(train_batch_data,train_batch_labels) in enumerate(trainloader):
        ...
        adv_inputs = adaad_inner_loss(student, teacher, train_batch_data, step_size=2/255,
                                        steps=10, epsilon=8/255)

        student.train()
        optimizer.zero_grad()

        ori_outputs = student(train_batch_data)
        adv_outputs = student(adv_inputs)

        with torch.no_grad():
            teacher.eval()
            t_ori_outputs = teacher(train_batch_data)
            t_adv_outputs = teacher(adv_inputs)
...
model_name = f'adaad_{args.teacher}_{args.student}_{args.epochs}ep_{save_time}.pt'
